...
const {projectionWithNewDocument} = require('../config/constants')
...
class User {
    ...
    async FindUserOrCreate() {
        if (this.controlInput(1) === true) {
            ...
            if (user) {
                return {title: 'Hypertube', message: `Sorry the ${this.data.username} or ${this.data.email} are already taken`, type: `register`}
            } else {
    ...
    async LogInOrRedirect() {
        if (this.controlInput(2)) {
            ...
            if (user) {
                try {
                    if (await argon2.verify(user.password, this.data.password))
                        return {user: user, message: `Hello ${this.data.username} very happy to see you`, type:`entryPoint`, title:`Hpertube`}
                    else
            ...
            } else return {type: `login`, message: `${this.data.username} not found or account nor vaildate`, title:`Hpertube`}
                ...
                if(user){
                    try {
                        ...
                        UserSchema.updateOne({
                            ...
                            .then(() => {
                                return { title: 'Hypertube', message: "Your password is modified with sucess, Log You now", type: 'login'}
                            })
                    }catch (e) {
        ...
        if (this.controlInput(3)) {
            ...
            if (query && id) {
                try {
                    ...
                    if (user) {
                        return {user: user, type: `Nice`, message: `Modification do with sucess`}
                    } else
            ...
            try{
                ...
                if(user){
                    try{
                        ...
                        return {type:`true`, message:`Instruction for reset pass are sent to ${this.data.email}`}
                    }catch (e) {
                        return{type:`False`, message:`The email ${this.data.email} doesn't match`}
                    }
                }
            }catch (e){
                return{type:`False`, message:`An Error Occured... try Later`}
            }
        }
    }
}
